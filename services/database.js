...
app.factory('database', function () {
    ...
    // JavaScript Promise pour récupérer les informations dans la base de données d'une certaine longueur
    var getCollectionLimitTo = function (collection, limite, idAExclure) {
        var liste = []
        var valeurs = [];
        console.log('enediart/' + collection);
        return new Promise(function (resolve, reject) {
            firebase.database().ref('enediart/' + collection + '/').once('value').then(function (snapshot) {
                if (snapshot != null && snapshot.val() != null) {
                    ...
                    while (liste.length < limite && tentative < limite * nombre) {
                        ...
                        if (isInArray(aleatoire, valeurs)) {

                        } else {
                            var a = element[Object.keys(element)[aleatoire]];
    ...
    const database = {
        ...
        getCollectionLimitTo: getCollectionLimitTo,
